...
class ContextManager:
    def __init__(
        ...
        if schema_mode == "static":
            ...
            if schema:
                ...
                self.graph_schema = extractor.extract_full_schema(
        ...
        for collection_name in self.resources:
            try:
                ...
                print(
                    f"Warning: Failed to initialize collection {collection_name}: {e}"
                )
    ...
    def _parse(self, result, role: Literal["user", "assistant"], resource: str):
        """
    ...
    async def load_context(
        self, query: str, from_resources: List[str]
    ) -> List[Dict[str, Any]]:
        """
        ...
        if self.schema_mode == "dynamic" and self.schema_retriever and context_messages:
            try:
                ...
                mapping_context = next(
                    (
                        msg
                        for msg in context_messages
                        if "mapping" in msg.get("content", "").lower()
                    ),
                    None,
                )
                if mapping_context:
                    schema_info = await self.schema_retriever.retrieve(
                        query, mapping_context["content"]
                    )
                    if schema_info:
    ...
    def _load_mapping_context(
        ...
        try:
            ...
            for resource in from_resources:
                if combined_results:
                    context_messages.append(
                        self._parse(combined_results, "user", resource)
                    )
            ...
            kg_logger.log_error(
                error_msg, {"query": query, "resources": from_resources}
            )
