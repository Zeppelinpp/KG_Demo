...
class KGLogger:
    ...
    def log_context_loading(self, query: str, resources: List[str], context_data: Dict[str, Any]):
        ...
        self.logger.info(f"[CONTEXT MANAGER] Context loaded items: {len(context_data) if context_data else 0}")
        if context_data:
            try:
                ...
                self.logger.info(f"[CONTEXT MANAGER] Context data: {json.dumps(serializable_data, ensure_ascii=False, indent=2)}")
            except Exception as e:
                self.logger.info(f"[CONTEXT MANAGER] Context data (raw): {str(context_data)}")
                self.logger.warning(f"[CONTEXT MANAGER] Failed to serialize context data: {e}")
        else:
            self.logger.info("[CONTEXT MANAGER] No context data loaded")
    
    def log_llm_call(self, system_prompt: str, user_query: str, model: str, messages: List[Dict[str, Any]]):
        ...
        self.logger.info(f"[LLM CALL] System Prompt: {system_prompt[:500]}{'...' if len(system_prompt) > 500 else ''}")
        self.logger.info(f"[LLM CALL] Message History Count: {len(messages)}")
        for i, msg in enumerate(messages):
            role = msg.get('role', 'unknown')
            content = msg.get('content', '')
            self.logger.info(f"[LLM CALL] Message {i+1} ({role}): {content[:200]}{'...' if len(content) > 200 else ''}")
    
    def log_schema_usage(self, schema: str):
        """Log graph schema information"""
        # Skip logging schema content as requested
        self.logger.info(f"[SCHEMA] Graph schema loaded, length: {len(schema)} characters")
    ...
    def log_tool_call(self, tool_name: str, tool_args: Dict[str, Any], tool_result: str):
        ...
        try:
            ...
            self.logger.info(f"[TOOL CALL] Arguments: {json.dumps(serializable_args, ensure_ascii=False, indent=2)}")
        except Exception as e:
    ...
    def log_error(self, error_msg: str, context: Dict[str, Any] = None):
        ...
        if context:
            try:
                serializable_context = self._make_serializable(context)
                self.logger.error(f"[ERROR] Context: {json.dumps(serializable_context, ensure_ascii=False, indent=2)}")
            except Exception as e:
